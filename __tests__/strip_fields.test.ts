...
describe("Strip field tests", () => {
  ...
  it('Keeps null fields when "keepEmptry" is true', () => {
    const result = deleteFields(
      ["fieldOne"],
      ["fieldOne", "fieldTwo", null, undefined],
      true,
      true
    );
  ...
  it("Sucessfully deletes nested property", () => {
    ...
    expect(
      result.fieldNested.nestedNestedField.fieldFour[0].fieldThree
    ).toBeDefined();
    expect(
      result.fieldNested.nestedNestedField.fieldFour[0].fieldTwo
    ).toBeDefined();
    expect(
      result.fieldNested.nestedNestedField.fieldFour[0].fieldOne
    ).toBeUndefined();
  });
});
